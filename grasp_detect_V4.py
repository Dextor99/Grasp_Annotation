...
def generate_cylinder_sections(pcd_points, frame, cyl_radius=75.0, offset=100.0, height=1.0):
    ...
    """
    origin = frame['origin']
    # origin = frame['origin'] + frame['x_axis'] * 30.0 - frame['y_axis'] * 10.0
    z_axis = frame['z_axis']
...
def filter_grippers_with_pointcloud_intersection(
    ...
    """
    对一批夹爪，根据它们与点云的交集是否存在，进行筛选，并将落在内部的点转换为夹爪坐标系下的坐标存储。
    """
    retained = []
    points_world = np.asarray(pcd.points)
    ...
    for i, g in enumerate(gripper_list):
        ...
        if len(indices) >= min_points_threshold:
            # 提取点的世界坐标
            inner_points_world = points_world[indices]  # (N, 3)

            # 将点从世界坐标系变换到夹爪局部坐标系
            T_world_gripper = np.linalg.inv(T)
            points_local = (T_world_gripper[:3, :3] @ inner_points_world.T + T_world_gripper[:3, 3:4]).T  # (N, 3)

            g_new = g.copy()
            g_new['inner_points_local'] = points_local  # 添加标签：夹爪局部系下的内部点
...
##########################OBB包围盒检测内部点云
    filtered = filter_grippers_with_pointcloud_intersection(min_opening_grippers, T_object_world, cloud_down, min_points_threshold=5)
    candidate_grippers_meshes = []
    for k in filtered:
        candidate_grippers_meshes.extend(k['meshes'])
