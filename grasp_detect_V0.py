...
def is_point_between_fingers1(point, T_world_gripper, opening, finger_length, finger_thickness=15):
    """
    ...
    in_x = (-opening / 2 + 1.0 <= x <= opening / 2 - 1.0)
    in_y = (-finger_thickness / 2 <= y <= finger_thickness / 2)
    in_z = (0 <= z <= finger_length)

    return in_x and in_y and in_z

def is_point_between_fingers(point, T_world_gripper, opening, finger_length, finger_thickness=15):
    """
    检查某点是否落在两指之间的包围区域（简化为盒子）。
    夹爪坐标系假设：
        - X 轴：从左指指心 -> 右指指心
    ...
    # 转换点到夹爪坐标系
    # T_gripper_world = np.linalg.inv(T_world_gripper)
    T_gripper_world =T_world_gripper
    p_local = T_gripper_world[:3, :3] @ point + T_gripper_world[:3, 3]  # R @ p + t

    x, y, z = p_local

    # 假设指间宽度为 opening，中间夹持区域 ±thickness/2，前后为 finger_length
    in_x = (-finger_thickness / 2 <= x <= finger_thickness / 2)
    in_y = (-opening / 2 + 1.0 <= y <= opening / 2 - 1.0)
    # in_z = (-finger_length <= z <= -2)
    in_z = (0 <= z <= finger_length)
...
def filter_grippers_with_object_between_fingers(T_object_world,grippers, point_cloud, finger_length=100):
    ...
    for g in grippers:
        ...
        found = False
        for pt in points:
            if is_point_between_fingers1(pt, T_world_gripper, opening, finger_length):
                found = True
                break
