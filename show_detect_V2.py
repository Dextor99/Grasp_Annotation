...
def generate_cylinder_sections(pcd_points, frame, cyl_radius=75.0, offset=100.0, height=1.0):
    ...
    """
    # origin = frame['origin']
    origin = frame['origin'] - frame['x_axis'] * 15.0 + frame['y_axis']
    z_axis = frame['z_axis']
...
def generate_gripper_variants(
    ...
    T_ref = T_object_world @ T_gripper_object
    variants = []
    idx = 0
    ...
    for angle_deg in range(0, max_deg + 1, step_deg):
        ...
        for opening in range(0, max_open + 1, step_open):
            ...
            gripper.transform(T_gripper_object_i)
            ...
            variants.append({
                ...
                'angle_deg': angle_deg,
                'opening': opening,
                'T_gripper_object': T_gripper_object_i.copy(),
                'meshes': meshes,
...
def slide_gripper_along_z(
    ...
    for d in range(0, max_distance + 1, step_mm):  # ✅ depth 在外层
        for variant in gripper_variants:           # ✅ variant 在内层
            ...
            gripper.opening = opening
            gripper.transform(T_gripper_object_shifted)
            ...
            moved_grippers.append({
                ...
                'opening': opening,
                'depth': d,
                'T_gripper_object': T_gripper_object_shifted.copy(),
                'meshes': gripper.get_meshes(),
                'origin': T_gripper_object_shifted[:3, 3],
                'model': gripper,
            })
...
def filter_grippers_by_box_volume(
    ...
    for g in gripper_list:
        ...
        visualize_gripper_box_and_pointcloud(box_corners_world, pcd)
...
def filter_grippers_with_pointcloud_intersection(
    ...
    """
    对一批夹爪，根据它们与点云的交集是否存在，进行筛选。
    参数:
        gripper_list: list[dict]，每个 dict 包含:
            - 'T_gripper_object': 4x4 numpy ndarray
            - 'opening': float
            - ... 其他字段将保留
        pcd: Open3D 点云对象（在世界坐标系下）
        min_points_threshold: 至少多少个点在夹爪内部，才保留

    返回:
        filtered_grippers: list[dict]，筛选后夹爪
    """
    retained = []
    for i, g in enumerate(gripper_list):
        ...
        visualize_obb_and_inner_points(pcd, obb)
...
#
# # ##########################OBB包围盒检测内部点云
# #     filtered = filter_grippers_with_pointcloud_intersection(min_opening_grippers, T_object_world, cloud_down, min_points_threshold=1)
# #     candidate_grippers_meshes = []
# #     for k in filtered:
# #         candidate_grippers_meshes.extend(k['meshes'])
    ...
    if cyl0 is not None and cyl1 is not None:
        ...
        # 显示第i个
        # vis_list = [cloud_down]
        vis_list = [cloud_down, cyl0, *object_world_axis,*transformed_gripper] + [projections[i - 1]]
        # vis_list = [cloud_down, cyl0, *object_world_axis] + [projections[i - 1]] + frame_arrows_list[
        #     i - 1] + filtered_meshes
        # 显示前i个
        # vis_list = [cloud_down, cyl0, cyl1, *object_world_axis] + projections[256:i]
...
if __name__ == "__main__":
    # ply_path = "model/0623/shuilongtou1_cut.ply"
