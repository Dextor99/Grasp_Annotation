...
def evaluate_grasp_final_score(candidate_grippers):
    """
    根据回归模型计算每个抓取姿态的最终评分，加入到字典中的 'final_score' 字段中。

    模型参数来自线性回归：
        final_score = 6.1774
                      + 0.6087 * score_force_closure
                      + 0.9362 * score_inner_points_ratio
                      + 0.9312 * score_proj_area_ratio
                      + 0.0892 * score_zmax
    """
    theta = {
        'intercept': 6.1774,
        'score_force_closure': 0.6087,
        'score_inner_points_ratio': 0.9362,
        'score_proj_area_ratio': 0.9312,
        'score_zmax': 0.0892
    }
...
def keep_top_k_grippers(candidate_grippers,top_k=10):
    """
    保留得分排名前top_k的夹爪及其mesh。

    参数:
        candidate_grippers: list[dict]，每个包含'final_score'字段
        candidate_grippers_meshes: list[open3d.geometry.TriangleMesh]
        top_k: int，保留的夹爪数量

    返回:
        top_grippers, top_meshes
    """
    # 加上索引，用于和mesh对应
    grippers_with_index = [(i, g) for i, g in enumerate(candidate_grippers) if
                           'final_score' in g and g['final_score'] is not None]

    # 按 final_score 排序（从大到小）
    grippers_with_index.sort(key=lambda x: x[1]['final_score'], reverse=True)

    # 保留前 top_k 项
    top_grippers_with_index = grippers_with_index[:top_k]

    # 解包
    top_indices = [idx for idx, _ in top_grippers_with_index]
    top_grippers = [candidate_grippers[idx] for idx in top_indices]
    top_meshes = []
    for k in top_grippers:
        top_meshes.extend(k['meshes'])
    return top_grippers, top_meshes
